...
def main(args):
    ...
    for idx, original_genes in enumerate(complexes, start=1):
        ...
        for ns in ("CC", "BP", "MF"):
            ...
            goea.ns2objgoea[ns].wr_tsv(out, res, pval_digits=58)
            print(f"  → {ns} results (all terms) written to {out}", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    p.add_argument("--complexes",  default="./complexes.txt",help="File: one complex per line")
    p.add_argument("--gaf",        default="../../data/Saccharomyces_cerevisiae/GO/sgd.gaf", help="GO annotation GAF file")
    p.add_argument("--obo",        default="../../data/Saccharomyces_cerevisiae/GO/go-basic.obo",help="GO ontology OBO file")
    p.add_argument("--background", help="Optional: file of background gene symbols")
    p.add_argument("--outdir",    default="../../data/results/GO_enrichments/collins_full", help="Directory for output TSVs")
    args = p.parse_args()
    main(args)
